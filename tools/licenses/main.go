...
type Entry struct {
	...
	Text        string
	PackageURL  string
	// Description is what the component IS, in one line — the sense CycloneDX
	// gives component.description. Curated per native lib (native.go) and empty
	// for Go modules, whose module path already names them and for which lit
	// holds no description it did not invent. Consumed by the SBOM only.
	Description string
	...
	// renders in LICENSE-REPORT.md's Notes section, in THIRD_PARTY_LICENSES,
	// and as a namespaced SBOM component property (licenseNoteProperty), so the
	// shipped artifacts explain themselves. It is NOT the component's
	// description: a licensing rationale is not what the component is.
	Note string
	...
	// of Note: the note's prose explains an election to a person, while a
