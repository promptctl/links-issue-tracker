...
is recorded unconditionally as a JSON record under `<traces_dir>/sync/`, whether
the command ran directly or under automation (a git hook, the on-change mirror,
the inline receive). This is separate from, and in addition to, the automation
trace under `<traces_dir>/automation/` described above: that one is written only
when `LNKS_AUTOMATION_TRIGGER` is set, so a directly-run interactive command left
no record there at all. A sync-trace record's `trigger` field is empty for an
