...
func TestRunQuickstartTextShowsNotReadyCountsWithoutIssueList(t *testing.T) {
	h := newReadyTestHarness(t)
	h.writeReadyConfig("description")
	...
	h.addBlocks(blocked.ID, blocker.ID)
	missing := h.createIssue(store.CreateIssueInput{
		Title:     "Missing description",
		Topic:     "docs",
		IssueType: "task",
		Priority:  2,
	...
	if strings.Contains(text, blocked.Title) {
		t.Fatalf("quickstart text should not list blocked issue title: %q", text)
	}
	if strings.Contains(text, missing.Title) {
		t.Fatalf("quickstart text should not list missing-field issue title: %q", text)
	}
}
