...
// carve-out riding through the shared seam: a mirror torn down by its own
// context (SIGTERM grace) still records the outcome, but an operator stopping
// a process is not the remote degrading, so the owner hook stays silent —
// identical to how a cancelled explicit push behaves.
func TestCompleteMirrorWithoutAttemptCancelledSkipsOwner(t *testing.T) {
	...
	if rec, _, ok := lastPushOutcome(ws, time.Now()); !ok || !rec.failed() {
		t.Fatalf("a cancelled mirror must still record its outcome (ok=%v rec=%+v)", ok, rec)
	}
