...
func commandSpecs(ctx context.Context, stdout io.Writer, stderr io.Writer) []CommandSpec {
	...
	return []CommandSpec{
		...
		{Name: "workflows", Summary: "See the work lifecycle and the guidance active at each point (`workflows show <id>` for one definition resolved)", GroupID: "guidance",
			Run: r.wsCmd(runWorkflows)},
