...
//
// [LAW:parse-dont-validate] Reconcile is a capability distinct from sync: an
// engine whose arrivals cannot conflict offers sync and declines this one
// (design.md §sync). Every subcommand below asks through here, so what each one
// holds is the interface itself and nothing inland re-asks.
//
// [LAW:single-enforcer] The decline is traced here too, under the command that
// needed it — four handlers recording the same refusal four ways is how the
// trace vocabulary drifts.
func reconcilerFor(ws workspace.Info, session syncSession, command string) (storage.Reconciler, error) {
...
func parseUnrelatedSide(arg string) (storage.UnrelatedResolution, error) {
	switch strings.ToLower(strings.TrimSpace(arg)) {
	case "local":
		return storage.TakeLocal, nil
	case "remote":
		return storage.TakeRemote, nil
	default:
...
func takeReasonForState(state storage.SyncReconcileState) string {
	switch state {
...
func reconcileCommandReasonForState(state storage.SyncReconcileState) string {
	switch state {
...
// [LAW:single-enforcer]
func reportTakeOutcome(stdout io.Writer, ws workspace.Info, command string, remote, branch string, result storage.SyncReconcileResult) error {
...
func discardedIDs(inv *storage.UnrelatedInventory, choice storage.UnrelatedResolution) []string {
	if inv == nil {
		return nil
	}
	switch choice {
...
func recordReconcileDecisionTrace(ws workspace.Info, command string, state storage.SyncReconcileState, metadata map[string]string) {
	recordSyncTraceLogged(ws, syncTraceRecord{
		Command:   command,
