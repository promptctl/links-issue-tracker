...
// collided instead of reporting a count.
type Collision struct {
	IssueID  string      `json:"issue_id"`
	Ours     model.Issue `json:"ours"`
	Theirs   model.Issue `json:"theirs"`
	OursWS   string      `json:"ours_workspace"`
	TheirsWS string      `json:"theirs_workspace"`
}
...
// unexported and Classify is its only constructor, which is what makes "field-merge
// a collision" unrepresentable rather than merely avoided by discipline.
...
func Classify(base, ours, theirs *model.Issue, oursWS, theirsWS string) (SameEntity, *Collision) {
