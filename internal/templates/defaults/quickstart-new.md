...
<agent-instructions>New tickets append to the bottom of their frame by default — the bottom of their epic's children, or of the top-level order for a ticket with no parent. Filing work records it; promoting it is a separate, deliberate act. Pass `--top` at creation for a ticket that genuinely belongs ahead of its siblings — first among its epic's children, or first in the top-level order for a ticket with no parent; `lit rank <id> --top` does the same later. A batch authored in order keeps that order with no flag.</agent-instructions>
