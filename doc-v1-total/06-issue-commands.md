...
Default active-work filter: if after merging no statuses **and no resolutions** are set, statuses default to `[open, in_progress]` — so a resolution filter alone reaches closed issues (`cli.go:587-589`).
