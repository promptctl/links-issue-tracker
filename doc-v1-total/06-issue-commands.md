...
3. **Annotators**, each attaching typed annotations:
   ...
   - *Blockers* — each non-closed dependency → `OpenDependency`; additionally `RankInversion` when the dependency ranks below its dependent. Each non-closed dependency of any epic above the issue, at any depth, that the issue does not already depend on directly → `InheritedDependency` (no rank inversion). A dependency that sits under that epic, or that the epic sits under, is skipped: the hierarchy already orders the two, and honoring the edge could leave issues waiting on each other forever. The ancestor epics, and the blockers with the epics above them, are loaded one batched query per nesting level (`ready_state.go:120-326`).
