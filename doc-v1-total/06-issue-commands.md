...
**`lit ls`** (`cli.go:380-570`). Lists issues, rank order by default. Uniquely, it is not bound to the current workspace: `--at <store-dir>` opens any discovered store read-only (blank or `-`-prefixed value → usage error). Filter flags: `--status` (a set: comma-separated and/or repeated, ORed), `--type` (single value), `--assignee`, `--search`, `--ids` (CSV), `--parent` (CSV of issue ids; selects their direct children, ORed; an explicitly empty value such as `--parent=` → usage error), `--labels` (CSV, ALL must match), `--has-comments` (tri-state: only applied when the flag is present, so `--has-comments=false` selects issues *without* comments), `--include-archived`, `--include-deleted`, `--updated-after`/`--updated-before` (RFC3339), `--sort` (`field[:asc|desc]` CSV), `--columns`, `--format` (`lines`|`table`; anything else exit 3), `--limit`, and `--query`.
