...
**`lit ls`** (`cli.go:440-663`). Lists issues, rank order by default. Uniquely, it is not bound to the current workspace: `--at <store-dir>` opens any discovered store read-only (blank or `-`-prefixed value → usage error). Filter flags: `--status` (a set: comma-separated and/or repeated, ORed), `--type` (single value), `--assignee`, `--search`, `--ids` (CSV), `--labels` (CSV, ALL must match), `--has-comments` (tri-state: only applied when the flag is present, so `--has-comments=false` selects issues *without* comments), `--include-archived`, `--include-deleted`, `--updated-after`/`--updated-before` (RFC3339), `--sort` (`field[:asc|desc]` CSV), `--columns`, `--format` (`lines`|`table`; anything else exit 3), `--limit`, and `--query`.

The `--query` language (`internal/query/query.go`) tokenizes with single/double quotes and accepts: `status:<state>[,<state>...]`, `resolution:<res>`, `type:<type>`, `assignee:<v>`, `id:<v>`, `label:<v>`, `has:comments`, `sort:<spec>`, `limit:<n>` (non-negative integer), bare `archived`, bare `deleted`, `updated>=|>|<=|<|:<RFC3339>`, and free text as search terms. Query and flag filters merge: slices dedupe or append, archived/deleted OR, a positive query limit overwrites; conflicting has-comments or time bounds, and after > before, are errors (`query.go:31-74, 276-309`).
...
**`lit next`** (read; `next.go`, `next_route.go`). Flags: `--assignee`, `--type`, `--status` (`open`|`in_progress`), `--labels`, and `--all` ("Ignore the focus scope and route over the whole queue") — no `--limit`, no `--columns` (`next.go:31`). The retired `--continue` is refused with an `UnsupportedError`. The command is registered `app.AccessRead` and performs no writes.

Admission is one predicate, `capacityFor` (`next_route.go:229`), which sorts a row into one of four capacities — `routeAround`, `serveWork`, `resumeWork`, `takeoverWork` — against the lane's standing and this checkout's identity. In a lane this checkout holds, an in-progress row yields `resumeWork`, a ready row `serveWork`, anything else `routeAround`. In a foreign lane a row is takeable when it is in progress and orphaned, or not in progress and ready; a row that is not takeable, and any row in a lane another checkout holds **fresh**, yield `routeAround`; an in-progress row, or any row in a **stale** lane, yields `takeoverWork`; the rest yield `serveWork`. A stale foreign lane is therefore a legitimate target of a bare `lit next` — only a fresh foreign hold is routed past. There is no `status == open` gate on servability, so an in-progress row in a lane this checkout holds is handed back to be resumed rather than hidden. Each step accepts a *set* of capacities and never ranks them against one another; composite rank is the only tiebreak routing applies.

`routeNext(rows, details, standings, self, scope)` (`next_route.go:307`) is the precedence. `ownScope` (`next_route.go:265`) derives the held lanes and epics from the **standings**, not from the gathered rows — rows are already narrowed by `--type`/`--labels`/`--assignee`, and deriving ownership from them let a display filter empty the set and drop the self-aware branch. A checkout holding at least one lane routes through steps 1–3; a checkout holding none starts at step 4.

- **Step 1** — own lanes, accepting `{serveWork, resumeWork}`, whichever the backlog ranks first: `resumeWork` yields `ResumedOwnWork{Row}`, `serveWork` yields `ServedFromClaim{Row}`.
- **Step 1b** — `onPathDependency` (`next_route.go:469`): a dependency outside our lanes that gates one of them. It establishes a claim on a lane we do not hold, so it yields `ServedFromNewLane{Row, Lane}` and is announced.
- **Step 2** — the rest of our epic, in lanes we do not already hold (`lane.Epic() != "" && ownEpics[lane.Epic()] && !mine(lane)`), accepting `{serveWork, takeoverWork}`: yields `ServedFromEpicLane{Row, Lane}`. The epic is `Lane.Epic()`; the outcome carries no separate epic field.
- **Step 3** — `Exhausted{Epics, Blocked}`: our own epic(s) have open work with none of it reachable. Exhaustion is terminal and **never** falls through to the global pool.
- **Step 4** — the focus-scoped global pool, reached only by a checkout holding no lanes. `scope.partition(rows)` splits the gathered rows into `pool` and `offPath`; the pool is walked accepting `{serveWork, takeoverWork}`, yielding `ServedFromNewLane{Row, Lane}`, and otherwise `NoWork{Unreachable}` where `Unreachable` is `passedOver(pool)` plus `withheldByScope(offPath)` — "nothing is startable" and "nothing on your focus path is startable" are different answers.

Steps 1–3 walk every gathered row; step 4 walks only the focus-scoped pool, and the row set is passed explicitly per step so that difference stays visible.

`NextOutcome` is a sealed sum (`isNextOutcome()`) of exactly those six cases. `Exhausted` and `NoWork` are themselves `error` implementations and travel outward as themselves rather than being rendered into a generic error, so the exit-code and reason sinks read the routing verdict instead of a copy. Both terminal outcomes exit 6 (`ExitNoWork`, `exit.go:30`) with reasons `scope_exhausted` and `no_ready_work` (`error_output.go:113,117`) — distinct from `ExitGeneric` so a caller looping `lit next` can separate "there is nothing for you" from "lit is broken" without parsing English, and not `ExitOK`, since for `lit next` 0 means a ticket is on stdout.

Both diagnostics are built from `reachKind`, what one row is to this checkout right now: `reachTakeable`, `reachHeldFresh`, `reachNotReady`, `reachOutOfView`, plus `reachOffFocusPath` used only by the pool diagnostic. Exhaustion asks it of the dependencies gating our scope (`gatingDependencies` `next_route.go:433`, `reachOf` `next_route.go:150`); an empty pool asks it of every row the walk went past. `maxNamedPerKind = 12`: one clause names at most twelve ids and states how many it left out. `exhaustedNotes`: "on your path and yours to take — `lit start` it", "on your path but claimed by another checkout right now", "on your path but not startable right now — `lit show` it", "on your path but outside this view — `lit show` it". `poolNotes`: "in progress or claimed in a lane another checkout holds right now", "not startable — blocked by a dependency, or in flight and not abandoned", "off the focus path this run answered over — `lit next --all` to route over the whole queue".

`Exhausted.Error()` (`next_route.go:480`) reads ``no ready work in %s — nothing else is queued behind what's already in progress; picking up other work is a deliberate re-focus, not a bare `next` `` when nothing is blocking, and substitutes `describeReach(o.Blocked, "blocked on ", exhaustedNotes)` for that middle clause when something is; the scope is `epic(s) <joined>` when epics are named, else `your claimed lane(s)`. `NoWork.Error()` (`next_route.go:587`) reads `no ready work` when nothing is unreachable, `no ready work on the focus path — the backlog is not empty, and each row below says why this run did not serve it: %s` under a focus scope, and `no ready work — the backlog is not empty, but nothing in it is startable here: %s` otherwise.

`lit next` claims nothing; the line it prints is advice about a command the reader has yet to run, produced by `startAdvice(row, lane, holder)` (`next.go:182`) in exactly four sentences. In progress, lane unnamed: "`<id>` is in progress and `<state>` — run `lit start <id>` to take it over"; in progress, lane named: "… to take over `<described>`"; not in progress, lane unnamed: "run `lit start <id>` to claim it"; not in progress, lane named: "run `lit start <id>` to claim `<described>`". `<state>` comes from `inFlightState(holder)`, one value of which is "abandoned". `LaneID.Describe()` (`model/model.go:255`) returns `("", false)` for a solo lane — a solo lane is the ticket that names it, so naming it only repeats the sentence — `("the default lane of epic <epic>", true)` for an empty key, and `("lane <key> of epic <epic>", true)` otherwise.

`renderNextOutcome` (`next.go:94`) prints per outcome: `ServedFromClaim`, no announcement at all; `ResumedOwnWork`, "`<id>` is already in progress in a lane you hold — continue where you left off"; `ServedFromEpicLane`, `startAdvice(...)` followed by " (a second lane of an epic you already hold a lane in)"; `ServedFromNewLane`, `startAdvice(...)`. `Exhausted` and `NoWork` are returned as themselves with nothing printed, and any other outcome panics.
