...
3. **Annotators**, each attaching typed annotations:
   ...
   - *Blockers* — each non-closed dependency → `OpenDependency`; additionally `RankInversion` when the dependency ranks below its dependent. Each non-closed dependency of any epic above the issue, climbing through nested epics and stopping at a parent that is not an epic, that the issue does not already depend on directly → `InheritedDependency` (no rank inversion). A dependency that sits under that epic, or that the epic sits under, is skipped: the hierarchy already orders the two, and honoring the edge could leave issues waiting on each other forever. The ancestor epics, and the blockers with the epics above them, are loaded one batched query per nesting level (`ready_state.go:120-326`).
   - *Sibling gate* — when the parent is an epic: each sibling with the same lane and a lower rank that is still in play → `EarlierSiblingPending`. The sibling set is the epic's **unfiltered** in-play children, so siblings hidden by CLI filters still gate (`ready_state.go:337-402`).
   - *Orphaned* — `in_progress` issues untouched for ≥ 6 hours (constant, `ready_state.go:49`).
   - *Needs-design* — the reserved label `needs-design` → `NeedsDesign`.
   - *Focus path* — issues on the prerequisite closure of any open goal labeled `focus`, computed by BFS over dependencies, dependencies inherited from ancestor epics, container children, and earlier same-lane siblings; shared prerequisites attribute to the first goal reached (`ready_state.go:456-673`).
...
5. **Ordering**, three stable sorts in sequence: composite rank (a leaf inside an epic sorts by the epic's rank, then its own), then priority (urgent first), then focus-path rows first — so focus outranks urgent (`cli.go:677-679`, `ready_state.go:715-762`).

Rollups partition rows as: `in_progress` first (even if also blocked), else blocked, else ready (`ready_state.go:807-820`).
...
`lit next` claims nothing, so nothing it prints is in the perfect tense. Any pick that would establish a claim prints advice about running `lit start`, worded by `startAdvice` (`next.go:183`) on whether the row is in progress and whether `LaneID.Describe()` (`model/model.go:255`) gives the lane a name — a solo lane gets none, being the ticket that names it — then the default columns plus indented `epic:`, `depends on:`, and claim lines, never `unblocks:`, since `printNextSummary` hands `printInlineDeps` a nil unblocks map (`ready_state.go:888-894`). `renderNextOutcome` (`next.go:94`) has an arm per case: `ServedFromClaim` (`:98`) announces nothing, the three other served outcomes announce (`:100-108`), the two terminal outcomes return themselves as errors (`:118-121`), and the unreachable `default` (`:123`) panics.
...
**`lit dep add --from <id> --to <id> [--type blocks|parent-child|related-to]`** (`dependency.go:24-69`). Default type `blocks`. Refusals in order: blank endpoints → usage (exit 2); bad type → bare error (exit 1); self-loop → error (exit 1); for `blocks` only, both endpoints in the same epic → validation error (exit 3): "Do not set 'blocks' relationships between two issues in the same epic.  Use rank…" (two floating issues never trip this); for `blocks` and `parent-child`, an edge that would leave unfinished issues waiting on each other forever → validation error (exit 3) naming each link of the loop, where the links are the ones readiness gates on: dependencies, blockers inherited from epics, an epic waiting on its children, and earlier lane-mates (`dependency.go:181-346`, `ready_state.go:531-620`). A loop that exists without the edge does not refuse it. A loop of `blocks` edges alone passes that check and is refused by the store (exit 1). Endpoints are swapped to storage orientation for `blocks` (chapter 01). Output lines: `<src> --blocks--> <dst>`, `--child-of-->`, `--related-to-->`.
