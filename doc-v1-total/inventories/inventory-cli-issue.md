...
`next` registers it too (`next.go:53`) and is the only read command that does:
...
- Read commands print `printStalenessWarning(ctx, w, ws, store, now)` FIRST,
  before their payload: `backlog` (`workable.go:185`), `next` (`next.go:68`),
...
| `--by` | string (hidden) | `""` | identity fallback (§1.11), read by `resumeAdvice` to say whether work in flight is the reader's (`next.go:53`) |

- `--status` and `--type` go through the same `parseWorkableStatus` /
  `parseWorkableType` refusals as `backlog` (`next.go:58-65`). **No** `--limit`,
  **no** `--columns`.
- Refusal: any positional → `UsageError{nextUsage}` → exit 2, where `nextUsage` =
  `"usage: lit next [--type ...] [--status ...] [--labels ...] [--assignee <user>] [--all]"`
  (`next.go:29`, `next.go:55-57`).
- Retired flag: `--continue` is intercepted at the shared parse boundary as
  ...
  (`flagset.go:138-141`).
- Prints the sync-staleness warning first (`next.go:68`).
- Gathers the workable set — rows, relation details, **and the focus scope** —
  via `gatherWorkableAnnotated` (`next.go:71-76`, `cli.go:655`), then the claim
  context (`next.go:80`), whose `actingAs` is then refined by this command's own
`--by` fallback (`next.go:88`), and then routes (`next.go:89`):
...
**Rendering** — `renderNextOutcome(w, outcome, details, cc)` (`next.go:112-162`):
- `ServedFromClaim` → no announcement at all (`next.go:116-117`).
- `ResumedOwnWork` → `resumeAdvice(o.Row, cc.actingAs)` + `"\n"` (`next.go:118-120`,
  `next.go:229-234`). The row's assignee decides which of two sentences: when it is
...
- `ServedFromDependency` → the same `startAdvice(...)` + `" (gates %s, which is in a lane you hold)\n"` on `Gates` (`next.go:132-135`).
- `Exhausted`, `NoWork` → returned as themselves; no ticket printed
  (`next.go:145-148`).
- Any other outcome type → panic (`next.go:149-150`).

`startAdvice(row, lane, holder)` (`next.go:260-273`) is exactly four sentences,
...
`<state>` is `inFlightState(holder)` (`next.go:176-184`): `claims.Locked` →
...
with `lane = model.LaneOf(row.Issue, details[row.ID].Parent)` (`next.go:157-160`),
...
`nextPulledOccasion(row.Issue)` (`next.go:161`, `workflow_events.go:39-45`),
dispatched as `EventNextPulled` (`next.go:93`).
...
7. **Three functions panic on unreachable states** and would abort the process:
   ...
   `renderNextOutcome` on an unhandled outcome type (`next.go:150`),
