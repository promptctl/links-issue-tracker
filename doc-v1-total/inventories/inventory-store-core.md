...
`nextRankForPlacement(ctx, tx, p storage.RankPlacement, f storage.Frame) (string, error)` (`store.go:2087-2095`): `edgeFor(p)` resolves the end — `storage.RankTop` → `topEdge`; `storage.RankBottom` → `bottomEdge`; anything else → `fmt.Errorf("unknown rank placement: %d", p)` (`internal/store/ranking.go:191-200`) — then `rankBetweenTx` returns a key between the bounds `edge.filingBoundsTx(ctx, tx, f)` reads (`internal/store/ranking.go:142-155`). `filingBoundsTx` reads that end's filing rank — frame `f`'s leading rank for the top (`frameEdgeRankTx`, `:166-169`), the whole workspace's last rank for the bottom (`workspaceEdgeRankTx`, `:176-185`) — and hands it to `e.roomBesideTx(ctx, tx, anchorRank, "")` (`:109-118`), which pairs it with the nearest rank the **whole workspace** holds on its far side; the `""` `excludeID` drops nothing from that read, since no issue's id is empty. A create at the top therefore takes the midpoint between the frame's leading rank and the nearest rank below it anywhere in the workspace; with no rank below it that read comes back `""` and the lower bound is open. An **empty** filing rank — a frame with nothing ranked in it — takes the other arm, which `filingBoundsTx` owns alone: it reads the workspace's last rank through `workspaceEdgeRankTx(ctx, tx, storage.TopLevel, bottomEdge)` and pairs it through `bottomEdge.roomBesideTx`, where nothing lies further out, so the key lands past the workspace's last. In an empty workspace both reads are `""`, so the pair is `("", "")` and the key is `rank.Initial()` ("V").
...
Called inside `CreateIssue`'s mutation right after ID minting — `internal/store/store.go:545-548`. The keyspace stays one flat space of rank strings across all issues; what is scoped to a frame is the existing key a new key lands beside — the filing frame's leading key at the top edge, the whole workspace's last key at the bottom. The key bounding that one on its far side is read from the whole workspace at both ends.
...
**RankAbove(issueID, targetID)** — `internal/store/ranking.go:868-902`:
- `resolveRankPair` first (all its errors propagate).
- In `withMutation(ctx, "rank above", …)`, the new rank comes from `rankBetweenTx`, whose `bounds` closure reads the anchor's rank through `anchorRankTx` and then returns `topEdge.roomBesideTx(ctx, tx, anchorRank, move.MovedID)` (`:886-892`). The closure's error → `fmt.Errorf("rank-above: %w", err)` (`:893-895`).
- `roomBesideTx` (`:109-118`) splices the edge's `outside` fragment into one shared statement (`:110-111`):
  ```sql
  SELECT item_rank FROM issues WHERE deleted_at IS NULL AND item_rank != '' AND id != ? AND item_rank < ? ORDER BY item_rank DESC LIMIT 1
  ```
  bound `(move.MovedID, anchorRank)`; `item_rank < ? ORDER BY item_rank DESC` is `topEdge.outside` (`:66`). Error → `fmt.Errorf("query the key outside the %s: %w", e.name, err)`, with `e.name` `"top"` (`:114`), which the `rank-above: %w` wrap prefixes. The read is **not** scoped to the pair's frame — it searches the whole workspace. The frame's say in the move is choosing the anchor, which `rankPairTx` already did. `id != ?` is the moving issue, which leaves the key it is about to vacate out of its own bounds, and `item_rank != ''` keeps unranked rows out.
- `topEdge.beside` orders the pair `(outsideRank, anchorRank)` (`:67`, `:116`), so the key found is the lower bound and the anchor the upper. Selecting nothing reads as `""` (`nearestRank`, `:1055-1065`), which happens only when nothing in the whole workspace sorts before the anchor; the lower bound is then open and the new rank is `rank.Midpoint("", anchorRank)`.
- The anchor's rank is read through `anchorRankTx` (`internal/store/ranking.go:824-834`): `SELECT item_rank FROM issues WHERE id = ? AND deleted_at IS NULL AND item_rank != ''`, so only a live, ranked anchor returns a row. No row → `fmt.Errorf("cannot rank against %s: it was deleted while the move was being applied, or it has no rank", anchorID)` (`:827-829`), which `bounds` and `rankBetweenTx` return unwrapped, so the `rank-above: %w` wrap prefixes it. An unranked or deleted anchor therefore fails the move.
- `writeRankTx(ctx, tx, move.MovedID, newRank, now)` with `now` from `s.clock` (`:896-899`); it runs `UPDATE issues SET item_rank = ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL`, error → `fmt.Errorf("set rank of %s: %w", id, err)` (`:390-402`).
- `smoothRanksIfNeededTx(ctx, tx, newRank)` (`:900`). Returns the `RankMove` regardless of which id the caller named.

**RankBelow(issueID, targetID)** — `internal/store/ranking.go:906-934`: mirror image — the `bounds` closure returns `bottomEdge.roomBesideTx(ctx, tx, anchorRank, move.MovedID)` (`:918-924`), whose `outside` fragment is `item_rank > ? ORDER BY item_rank ASC` (`:73`) and whose `e.name` is `"bottom"`; `bottomEdge.beside` orders the pair `(anchorRank, outsideRank)` (`:74`), so the anchor is the lower bound and the key found the upper. The closure's error → `fmt.Errorf("rank-below: %w", err)` (`:925-927`); the write is the same `writeRankTx` (`:929-931`) and the smoothing the same `smoothRanksIfNeededTx` (`:932`). This read is likewise unscoped to the frame, so it reads `""` only when nothing in the whole workspace sorts after the anchor, and the new rank is then `rank.Midpoint(anchorRank, "")`. The anchor's rank is read through the same `anchorRankTx` (`:824-834`), so an unranked or deleted anchor fails the move with `"rank-below: cannot rank against %s: it was deleted while the move was being applied, or it has no rank"`.
...
`rankEdge.rankBeyondTx(ctx, tx, movedID, readEdge)` — `internal/store/ranking.go:217-225`: the key past a frame's edge. Its body is one `rankBetweenTx` call, whose `bounds` reads the edge's rank through `readEdge()` on every call, returns a `readEdge` error as is, and hands the rank to `e.roomBesideTx(ctx, tx, edgeRank, movedID)` (`:218-224`). `roomBesideTx` (`:109-118`) runs `SELECT item_rank FROM issues WHERE deleted_at IS NULL AND item_rank != '' AND id != ? AND <e.outside> LIMIT 1` bound `(movedID, edgeRank)` — `item_rank < ? ORDER BY item_rank DESC` for `topEdge`, `item_rank > ? ORDER BY item_rank ASC` for `bottomEdge` — and orders the pair through the edge's `beside`: `topEdge` gives `(outsideRank, edgeRank)` and `bottomEdge` gives `(edgeRank, outsideRank)` (`:63-76`), the two values `edgeFor` returns (`:191-200`). `movedID` keeps the moving issue's own key out of its bounds; a failed read is wrapped `fmt.Errorf("query the key outside the %s: %w", e.name, err)` (`:114`). Nothing on the far side reads as `""`, the open bound. A pair at either end that pads to the same value gets room made the same way a relative move's pair does, and because the edge is not read ahead of `rankBetweenTx` the second read sees the respaced ranks. An empty frame's edge reads as `""` and goes to `roomBesideTx` unchanged — the empty-population rule that files past the workspace's last key is `filingBoundsTx`'s alone (`:142-155`), and `rankBeyondTx` has no such arm. At the top edge nothing sorts below the empty string, so the far-side read is `""` as well, both bounds are empty, and `rank.Midpoint("", "")` returns `rank.Initial()`'s `"V"` (`internal/rank/rank.go:85-143`, `:39-41`); at the bottom edge every live rank sorts above the empty string, so the read returns the workspace's first rank outside `movedID` and the pair is `("", firstRank)`. Its callers are `rankToEdge`, passing `issueID` as `movedID`, reading the frame's edge holder through `frameEdgeHolderTx` and wrapping the error `fmt.Errorf("rank to %s: %w", edge.name, err)` (`:466-472`); `RankSet`, passing `""`, for the key of its last id (`:591-597`).
