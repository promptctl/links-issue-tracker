...
`nextRankForPlacement(ctx, tx, p storage.RankPlacement, f storage.Frame) (string, error)` (`store.go:2087-2095`): `edgeFor(p)` resolves the end — `storage.RankTop` → `topEdge`; `storage.RankBottom` → `bottomEdge`; anything else → `fmt.Errorf("unknown rank placement: %d", p)` (`internal/store/ranking.go:184-193`) — then `rankBetweenTx` returns a key between the bounds `edge.filingBoundsTx(ctx, tx, f)` reads (`internal/store/ranking.go:142-148`). `filingBoundsTx` reads that end's filing rank — frame `f`'s leading rank for the top (`frameEdgeRankTx`, `:159-162`), the whole workspace's last rank for the bottom (`workspaceEdgeRankTx`, `:169-178`) — and hands it to `boundsBesideTx` (`:113-122`), which pairs it with the nearest rank the **whole workspace** holds on its far side (`roomBesideTx`, `:129-138`). A create at the top therefore takes the midpoint between the frame's leading rank and the nearest rank below it anywhere in the workspace; with no rank below it that read comes back `""` and the lower bound is open. An **empty** filing rank — a frame with nothing ranked in it — takes the other arm: `boundsBesideTx` reads the workspace's last rank and pairs it through `bottomEdge.roomBesideTx`, where nothing lies further out, so the key lands past the workspace's last. In an empty workspace both reads are `""`, so the pair is `("", "")` and the key is `rank.Initial()` ("V").
...
**RankAbove(issueID, targetID)** — `internal/store/ranking.go:340-366`:
...
- The anchor's rank is read through `anchorRankTx` (`internal/store/ranking.go:649-659`): `SELECT item_rank FROM issues WHERE id = ? AND deleted_at IS NULL AND item_rank != ''`, so only a live, ranked anchor returns a row. No row → `fmt.Errorf("cannot rank against %s: it was deleted while the move was being applied, or it has no rank", anchorID)` (`:652-654`), which `bounds` and `rankBetweenTx` return unwrapped, so the `rank-above: %w` wrap prefixes it. An unranked or deleted anchor therefore fails the move.
...
**RankBelow(issueID, targetID)** — `internal/store/ranking.go:370-396`: mirror image — `item_rank > ? … ORDER BY item_rank ASC LIMIT 1`, errors `"query neighbor: %w"` (`:379`) and `"rank-below: %w"` around the `rankBetweenTx` error, which carries it (`:387`), `"rank-below: update: %w"` (`:391`); the anchor's rank is the lower bound and the neighbor the upper, and no neighbor reads as `""`, so at the frame's bottom the new rank is `rank.Midpoint(anchorRank, "")`. The anchor's rank is read through the same `anchorRankTx` (`:649-659`), so an unranked or deleted anchor fails the move with `"rank-below: cannot rank against %s: it was deleted while the move was being applied, or it has no rank"`.
...
`rankEdge.rankBeyondTx(ctx, tx, readEdge)` — `internal/store/ranking.go:211-219`: the key past a frame's edge. Its body is one `rankBetweenTx` call, whose `bounds` reads the edge's rank through `readEdge()` on every call, returns a `readEdge` error as is, and hands the rank to `e.boundsBesideTx` (`:212-218`). That pairs the rank with the nearest rank the **whole workspace** holds on its far side, read by `roomBesideTx` (`:129-138`) through the edge's `outside` fragment — `item_rank < ? ORDER BY item_rank DESC` for `topEdge`, `item_rank > ? ORDER BY item_rank ASC` for `bottomEdge` — and ordered by the edge's `beside`: `topEdge` gives `(outsideRank, edgeRank)` and `bottomEdge` gives `(edgeRank, outsideRank)` (`:63-76`), the two values `edgeFor` returns (`:184-193`). Nothing on the far side reads as `""`, the open bound; a failed read is wrapped `fmt.Errorf("query the key outside the %s: %w", e.name, err)` (`:134`). A pair at either end that pads to the same value gets room made the same way a relative move's pair does, and because the edge is not read ahead of `rankBetweenTx` the second read sees the respaced ranks. An empty frame's edge reads as `""`, and `boundsBesideTx` then files against the workspace's last rank instead (`workspaceEdgeRankTx`, `:169-178`, paired through `bottomEdge.roomBesideTx`); in an empty workspace that read is `""` too and nothing lies outside it, so both bounds are empty and `rank.Midpoint("", "")` returns `rank.Initial()`'s `"V"` (`internal/rank/rank.go:85-143`, `:39-41`). Its callers are `rankToEdge`, reading the frame's edge holder through `frameEdgeHolderTx` and wrapping the error `fmt.Errorf("rank to %s: %w", edge.name, err)` (`:460-466`); `RankSet`, for the key of its last id (`:585-591`).
