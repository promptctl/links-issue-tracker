...
1. `frame, err := filingFrameTx(ctx, tx, parentID)` (`store.go:526`; defined `internal/store/ranking.go:350-366`): an empty `parentID` → `storage.TopLevel`; otherwise `SELECT deleted_at FROM issues WHERE id = ?`, with `sql.ErrNoRows` → `storage.NotFoundError{Entity: "issue", ID: parentID}`, other error → `fmt.Errorf("lookup parent issue %q: %w", parentID, err)`, a non-NULL `deleted_at` → `storage.TopLevel`, and a live parent → `storage.Frame(parentID)`.
...
`nextRankForPlacement(ctx, tx, p storage.RankPlacement, f storage.Frame) (string, error)` (`store.go:2088-2096`): `edgeFor(p)` resolves the end — `storage.RankTop` → `topEdge`; `storage.RankBottom` → `bottomEdge`; anything else → `fmt.Errorf("unknown rank placement: %d", p)` (`internal/store/ranking.go:248-257`) — then `rankBetweenTx` returns a key between the bounds `edge.filingBoundsTx(ctx, tx, f)` reads (`internal/store/ranking.go:203-212`). `filingBoundsTx` reads that end's filing rank — frame `f`'s leading rank for the top (`frameEdgeRankTx`, `:223-226`), the whole workspace's last rank for the bottom (`workspaceEdgeRankTx`, `:233-242`) — and hands it to `e.roomBesideTx(ctx, tx, anchorRank)` (`:111-144`), which pairs it with the nearest rank the **whole workspace** holds on its far side; a create names no moving ids, so the variadic `moving` is empty and the statement carries no `AND id NOT IN (...)` clause at all (`:131-137`). A create at the top therefore takes the midpoint between the frame's leading rank and the nearest rank below it anywhere in the workspace; with no rank below it that read comes back `""` and the lower bound is open. An **empty** filing rank — a frame with nothing ranked in it — never reaches that read: `roomBesideTx` refuses an empty anchor outright with `fmt.Errorf("no room beside the %s of this frame: the key it was read from is empty", e.name)` (`:122-124`), and `filingBoundsTx` routes the case to `firstInFrameBoundsTx(ctx, tx, f)` (`:208-210`, `:167-191`) instead. That reads the rank of the issue `f` names — `SELECT item_rank FROM issues WHERE id = ? AND deleted_at IS NULL AND item_rank != ''`, error → `fmt.Errorf("query the rank of frame %q: %w", f, err)` (`:171-175`) — and returns `bottomEdge.roomBesideTx(ctx, tx, containerRank)` (`:190`), so the frame's first issue lands just past its container's own key. `storage.TopLevel` returns `("", "")` directly (`:168-170`), which is `rank.Initial()` ("V") — the top level holds nothing ranked only when the whole workspace does. A container carrying no rank of its own falls back to the workspace's last rank through `workspaceEdgeRankTx(ctx, tx, storage.TopLevel, bottomEdge)`, or to `("", "")` when that is empty too (`:180-189`).
...
**RankAbove(issueID, targetID)** — `internal/store/ranking.go:933-967`:
- `resolveRankPair` first (all its errors propagate).
- In `withMutation(ctx, "rank above", …)`, the new rank comes from `rankBetweenTx`, whose `bounds` closure reads the anchor's rank through `anchorRankTx` and then returns `topEdge.roomBesideTx(ctx, tx, anchorRank, move.MovedID)` (`:951-957`). The closure's error → `fmt.Errorf("rank-above: %w", err)` (`:958-960`).
...
- `topEdge.beside` orders the pair `(outsideRank, anchorRank)` (`:67`, `:142`), so the key found is the lower bound and the anchor the upper. Selecting nothing reads as `""` (`nearestRank`, `:1120-1130`), which happens only when nothing in the whole workspace sorts before the anchor; the lower bound is then open and the new rank is `rank.Midpoint("", anchorRank)`.
...
- `writeRankTx(ctx, tx, move.MovedID, newRank, now)` with `now` from `s.clock` (`:961-964`); it runs `UPDATE issues SET item_rank = ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL`, error → `fmt.Errorf("set rank of %s: %w", id, err)` (`:455-467`).
- `smoothRanksIfNeededTx(ctx, tx, newRank)` (`:965`). Returns the `RankMove` regardless of which id the caller named.

**RankBelow(issueID, targetID)** — `internal/store/ranking.go:971-999`: mirror image — the `bounds` closure returns `bottomEdge.roomBesideTx(ctx, tx, anchorRank, move.MovedID)` (`:983-989`), whose `outside` fragment is `item_rank > ? ORDER BY item_rank ASC` (`:73`) and whose `e.name` is `"bottom"`; `bottomEdge.beside` orders the pair `(anchorRank, outsideRank)` (`:74`), so the anchor is the lower bound and the key found the upper. The closure's error → `fmt.Errorf("rank-below: %w", err)` (`:990-992`); the write is the same `writeRankTx` (`:993-996`) and the smoothing the same `smoothRanksIfNeededTx` (`:997`). This read is likewise unscoped to the frame, so it reads `""` only when nothing in the whole workspace sorts after the anchor, and the new rank is then `rank.Midpoint(anchorRank, "")`. The anchor's rank is read through the same `anchorRankTx` (`:889-899`), so an unranked or deleted anchor fails the move with `"rank-below: cannot rank against %s: it was deleted while the move was being applied, or it has no rank"`.
