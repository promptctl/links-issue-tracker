...
**RankAbove(issueID, targetID)** — `internal/store/ranking.go:893-927`:
- `resolveRankPair` first (all its errors propagate).
- In `withMutation(ctx, "rank above", …)`, the new rank comes from `rankBetweenTx`, whose `bounds` closure reads the anchor's rank through `anchorRankTx` and then returns `topEdge.roomBesideTx(ctx, tx, anchorRank, move.MovedID)` (`:911-917`). The closure's error → `fmt.Errorf("rank-above: %w", err)` (`:918-920`).
- `roomBesideTx` (`:109-131`) splices the edge's `outside` fragment into one shared statement (`:123-124`):
  ```sql
  SELECT item_rank FROM issues WHERE deleted_at IS NULL AND item_rank != '' AND id != ? AND item_rank < ? ORDER BY item_rank DESC LIMIT 1
  ```
  bound `(move.MovedID, anchorRank)`; `item_rank < ? ORDER BY item_rank DESC` is `topEdge.outside` (`:66`). Error → `fmt.Errorf("query the key outside the %s: %w", e.name, err)`, with `e.name` `"top"` (`:127`), which the `rank-above: %w` wrap prefixes. The read is **not** scoped to the pair's frame — it searches the whole workspace. The frame's say in the move is choosing the anchor, which `rankPairTx` already did. `id != ?` is the moving issue, which leaves the key it is about to vacate out of its own bounds, and `item_rank != ''` keeps unranked rows out.
- `topEdge.beside` orders the pair `(outsideRank, anchorRank)` (`:67`, `:129`), so the key found is the lower bound and the anchor the upper. Selecting nothing reads as `""` (`nearestRank`, `:1080-1090`), which happens only when nothing in the whole workspace sorts before the anchor; the lower bound is then open and the new rank is `rank.Midpoint("", anchorRank)`.
- The anchor's rank is read through `anchorRankTx` (`internal/store/ranking.go:849-859`): `SELECT item_rank FROM issues WHERE id = ? AND deleted_at IS NULL AND item_rank != ''`, so only a live, ranked anchor returns a row. No row → `fmt.Errorf("cannot rank against %s: it was deleted while the move was being applied, or it has no rank", anchorID)` (`:852-854`), which `bounds` and `rankBetweenTx` return unwrapped, so the `rank-above: %w` wrap prefixes it. An unranked or deleted anchor therefore fails the move.
- `writeRankTx(ctx, tx, move.MovedID, newRank, now)` with `now` from `s.clock` (`:921-924`); it runs `UPDATE issues SET item_rank = ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL`, error → `fmt.Errorf("set rank of %s: %w", id, err)` (`:415-427`).
- `smoothRanksIfNeededTx(ctx, tx, newRank)` (`:925`). Returns the `RankMove` regardless of which id the caller named.

**RankBelow(issueID, targetID)** — `internal/store/ranking.go:931-959`: mirror image — the `bounds` closure returns `bottomEdge.roomBesideTx(ctx, tx, anchorRank, move.MovedID)` (`:943-949`), whose `outside` fragment is `item_rank > ? ORDER BY item_rank ASC` (`:73`) and whose `e.name` is `"bottom"`; `bottomEdge.beside` orders the pair `(anchorRank, outsideRank)` (`:74`), so the anchor is the lower bound and the key found the upper. The closure's error → `fmt.Errorf("rank-below: %w", err)` (`:950-952`); the write is the same `writeRankTx` (`:953-956`) and the smoothing the same `smoothRanksIfNeededTx` (`:957`). This read is likewise unscoped to the frame, so it reads `""` only when nothing in the whole workspace sorts after the anchor, and the new rank is then `rank.Midpoint(anchorRank, "")`. The anchor's rank is read through the same `anchorRankTx` (`:849-859`), so an unranked or deleted anchor fails the move with `"rank-below: cannot rank against %s: it was deleted while the move was being applied, or it has no rank"`.
...
`rankEdge.rankBeyondTx(ctx, tx, movedID, readEdge)` — `internal/store/ranking.go:234-250`: the key past a frame's edge. Its body is one `rankBetweenTx` call, whose `bounds` reads the edge's rank through `readEdge()` on every call and returns a `readEdge` error as is (`:235-249`). A non-empty rank goes to `e.roomBesideTx(ctx, tx, edgeRank, movedID)` (`:109-131`), which runs `SELECT item_rank FROM issues WHERE deleted_at IS NULL AND item_rank != '' AND id != ? AND <e.outside> LIMIT 1` bound `(movedID, edgeRank)` — `item_rank < ? ORDER BY item_rank DESC` for `topEdge`, `item_rank > ? ORDER BY item_rank ASC` for `bottomEdge` (`:123-124`) — and orders the pair through the edge's `beside`: `topEdge` gives `(outsideRank, edgeRank)` and `bottomEdge` gives `(edgeRank, outsideRank)` (`:63-76`), the two values `edgeFor` returns (`:208-217`). `movedID` keeps the moving issue's own key out of its bounds; a failed read is wrapped `fmt.Errorf("query the key outside the %s: %w", e.name, err)` (`:127`). Nothing on the far side reads as `""`, the open bound. A pair at either end that pads to the same value gets room made the same way a relative move's pair does, and because the edge is not read ahead of `rankBetweenTx` the second read sees the respaced ranks. An **empty** edge rank takes the other arm: `bounds` returns `pastTheWorkspaceTx(ctx, tx)` (`:245-247`, `:142-151`), so a frame holding nothing ranked but the moving issue files past the workspace's last key rather than into the middle of the keyspace — `rank.Initial()`'s `"V"` only when the workspace itself holds no rank, which is the `("", "")` `pastTheWorkspaceTx` returns for an empty last read (`internal/rank/rank.go:85-143`, `:39-41`). The empty rank never reaches `roomBesideTx`, which refuses an empty anchor with `fmt.Errorf("no room beside the %s of this frame: the key it was read from is empty", e.name)` (`:120-122`). Both callers get that arm: `rankToEdge`, passing `issueID` as `movedID`, reading the frame's edge holder through `frameEdgeHolderTx` and wrapping the error `fmt.Errorf("rank to %s: %w", edge.name, err)` (`:491-497`); `RankSet`, passing `""`, for the key of its last id (`:616-622`).
