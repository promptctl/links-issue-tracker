...
**no event at all** — the type assertion at `internal/cli/cli.go:1415` excludes them
...
| `internal/cli/cli.go:886` (`runShow`) | `show_ticket` | after `GetIssueDetail`, **before** either the `--field` output or the full detail view — fires for both (`internal/cli/cli.go:885`, `:891-897`) |
| `internal/cli/cli.go:1033` (`runUpdate`) | `ticket_updated` | after `Store.Apply`, before summary/breadcrumb |
| `internal/cli/cli.go:1416` (`runTransition`) | one of the four transition events | after `Store.Apply` and after `authorize`; **before** the claim-transfer notice at `internal/cli/cli.go:1424-1427`. Guarded by `action.(model.StatusAction)` (`internal/cli/cli.go:1415`) |
| `internal/cli/cli.go:1491` (`runCommentAdd`) | `comment_added` | after `AddComment`, before `printComment` |
...
- **A write failure to `w` aborts and returns the error**, which the calling command returns
  directly (`internal/workflows/dispatch.go:64-66`; e.g. `internal/cli/cli.go:1416-1418`).
...
- Its error return **propagates as the command's error** at every call site
  (`internal/cli/cli.go:359-361`, `:431-433`, `:886-888`, `:1033-1035`, `:1416-1418`, `:1491-1493`;
