...
**`place(id, f, placement)`** (`internal/storage/memory/issues.go:121-150`) — `RankTop` takes the first slot among the frame `f`'s members; `RankBottom` (the zero value) appends to the whole order; any other value → `fmt.Errorf("unknown rank placement: %d", p)` from `orderEdgeFor`. A population with no members takes neither end: `place` calls `e.insertAt(e.slotInsideContainer(f), id)` and returns (`:135-147`), so the first member of a frame lands immediately after the issue that frames it — `slices.Index(e.order, string(f))` plus one (`:158-167`). `storage.TopLevel` names no such issue and gives slot `0`; a container absent from the order gives `len(e.order)`.
