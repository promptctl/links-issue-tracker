...
**`place(id, f, placement)`** (`internal/storage/memory/issues.go:121-154`) — `RankTop` takes the first slot among the frame `f`'s members; `RankBottom` (the zero value) appends to the whole order; any other value → `fmt.Errorf("unknown rank placement: %d", p)` from `orderEdgeFor`. A population with no members takes neither end: `place` takes the slot from `e.slotInsideContainer(f)`, propagates its error, and otherwise inserts there (`:135-151`), so the first member of a frame lands immediately after the issue that frames it — `slices.Index(e.order, string(f))` plus one (`:162-178`). `storage.TopLevel` names no such issue and gives slot `0`; a frame absent from the order is refused with `fmt.Errorf("frame %s holds no position in the order; an issue cannot be filed inside one that is not there", f)` (`:174-176`).
