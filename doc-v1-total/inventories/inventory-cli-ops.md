...
- `internal/cli/cli.go:274-300` `parseFlagSet`:
  ...
  - `--continue` → `UnsupportedError{Message: "--continue is retired; claim routing already keeps \`lit next\` in your checkout's own epic first — run \`lit next\` with no flag"}` (exit 3) — `flagset.go:134-136`.
  - Any other `unknown flag:` / `unknown shorthand flag:` / `flag needs an argument:` → `UsageError` (exit 2) — `flagset.go:143-146`.
...
Read commands that additionally print the store-backed banner: `internal/cli/cli.go:916`, `internal/cli/next.go:55`, `internal/cli/workable.go:185` (i.e. `show`-family, `next`, and `backlog`/workable views).
...
2. Only when `info.BuildAge(now)` reports `ok` (a real, past, parsed date): `built <coarse duration> ago` (`version.go:53-57`).
...
| Unknown flag | any command | `UsageError`, exit 2 | `flagset.go:143-146` |
| `--output` before the command name; `--continue` | any command | `UnsupportedError`, exit 3 | `cli.go:196-201`, `flagset.go:134-136` |
