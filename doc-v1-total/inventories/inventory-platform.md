...
| `CLAUDE_CODE_SESSION_ID` | `internal/cli/cli.go:1180` | When non-empty (after trim), the acting identity is always `claude_<sessionID>`, overriding `--assignee`/`--by`; otherwise the trimmed explicit value passes through (`internal/cli/cli.go:1179-1184`). |
...
`CLAUDE_CODE_SESSION_ID` (`internal/cli/cli.go:1180-1181`).
