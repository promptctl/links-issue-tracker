...
and names `UpstreamIndexURL` as its source. Its keys, in upstream order,
2 FRAMING + 21 LAW:
...
namespace alternation built by `namespaceAlternation` (`markers.go:37-43`) from
the `namespaces` table in `index.go:23-29`. The
...
`ScanMarkers(content string) []Marker` — `markers.go:49-61`: splits `content` on
...
`NonCanonical(markers []Marker) []Marker` — `markers.go:64-72`: the subset whose
...
## 8.3 Upstream index parsing — `index.go`

`UpstreamIndexURL` — `index.go:13`:
`https://raw.githubusercontent.com/promptctl/laws/master/plugins/laws/skills/code/SKILL.md`.

`namespaces` — `index.go:23-29`: `FRAMING` under the paragraph header prefix
...
longer upstream. `.github/workflows/nightly.yml` runs the `-check` form.
