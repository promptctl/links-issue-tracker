...
**`startAdvice(row, lane, holder)`** (`internal/cli/next.go:242-255`) — the line every pick that would establish a claim prints above its row: what running `lit start` would lock, never what this command did. `lit next` claims nothing and starts nothing; the function was `claimAnnouncement` and the rename is the fix, an announcement reporting being the one thing a read-only command must not do (`internal/cli/next.go:89-93`, `:218-221`). `described, named := lane.Describe()`; exactly four sentences:
...
- `ResumedOwnWork` → `resumeAdvice(o.Row, cc.actingAs)` + `"\n"`. Two sentences, chosen by
  ...
  (`internal/cli/next.go:211-216`). Both halves must be non-empty and differ, so an
