...
| Dolt journal | `<db>/links/.dolt/noms/LOCK` | exclusive | `coResidentHolderWait` ÷ `doltJournalRetryDelay` = 700 × 100ms = 70s | Dolt's own file; `LockDoltJournalExclusive` stats first (absent → `repository not initialized with lit`) and never mints Dolt's tree |
