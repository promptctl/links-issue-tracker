...
| `lit show` (`cli.go:905`) | `show_ticket` | after detail load, before either `--field` output or the full view — fires for both |
| `lit update` (`cli.go:1056`) | `ticket_updated` | after apply, before summary |
| transitions (`cli.go:1439`) | one of the four transition events | after apply and authorize, before the claim-transfer notice |
| `lit comment` (`cli.go:1514`) | `comment_added` | after the comment is stored, before it is printed |
...
| `lit backlog` (`internal/cli/workable.go:247`) | `show_backlog` | last, after the table render |
