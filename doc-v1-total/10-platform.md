...
| `CLAUDE_CODE_SESSION_ID` | Non-empty after trim → the acting identity is always `claude_<sessionID>`, overriding `--assignee`/`--by` (`cli.go:1202-1207`) |
...
`<git-common-dir>/links/config.json` holds `workspace_id`, `issue_prefix`, `created_at` (RFC3339), `schema_version` (`internal/workspace/workspace.go:72-77`). Created on first resolve with a fresh UUID, UTC now, version 1 (`workspace.go:613-618`). Writes are atomic (temp file + chmod 0644 + rename); `UpdateConfig` is the single read-modify-write boundary (`workspace.go:668-682`). A blank `issue_prefix` is filled from one of two sources, ranked by `resolveIssuePrefix`: an explicit `workspace.PrefixRequest` if the caller carries one (`lit init --prefix`), otherwise derivation from the repository directory name — normalize the basename, split on `-`, take the first part that normalizes to a valid prefix, else the whole normalized base. Either way the value is persisted back immediately and reads back as *configured*, not derived, when it came from a request. Derivation failing is `ErrIssuePrefixRefused`, exit 3; so is a request that contradicts a non-blank stored prefix, which is refused rather than applied — `lit prefix set` is the command that changes a prefix already in use. The third and least escapable case, a STORED `issue_prefix` the rules refuse, is the typed `StoredPrefixError`: it unwraps to the same sentinel and so shares exit 3, but classifies as its own reason `stored_prefix_refused`, because no command clears it — `lit prefix set` and `lit doctor` both resolve the workspace first and die in the same place — so both the message and the remediation name the config file rather than any command (`workspace.go:520-556`, `:63-68`, `internal/cli/error_output.go:221-228`).

## Workspace discovery

### One workspace from cwd

A `Location` is pure path geometry derived from the storage dir (`workspace.go:376-387`): `StorageDir` = `<git-common-dir>/links`, `ConfigPath` = `<storage>/config.json`, `DatabasePath` = `<storage>/dolt`, `DoltRepoPath` = `<storage>/dolt/links`.

`deriveLocation(cwd)` (`workspace.go:280-316`): run `git rev-parse --git-common-dir` anchored at cwd; join a relative answer onto the absolute cwd (not the repo toplevel); canonicalize through `filepath.EvalSymlinks`; append `links`. `Resolve(cwd)` adds: `git rev-parse --show-toplevel` for the root, `git rev-parse --git-dir` for the checkout's **private** git dir (deliberately not symlink-canonicalized, `workspace.go:353-367`), `MkdirAll` on the storage dir, and load-or-create of `config.json` (`workspace.go:221-223`, `:257-260`). All geometry git calls use `context.Background()` deliberately.

Git-error classification (`workspace.go:429-435`): only an `exec.ExitError` with exit code 128 maps to the sentinel `ErrNotGitRepo`; git missing from PATH, signal-killed git, or any other exit code is surfaced with context.
...
Link-time variables `Version`, `Commit`, `Date`, `Origin` are stamped by three writers: goreleaser (all four, `Origin` the literal `release`), `scripts/install.sh` source mode (all four, `Version` via `git describe`), and the Justfile `build` recipe (Commit + Date + Origin only — a `just build` binary is a dev build) (`internal/version/version.go:23-46`). Two of those four fields are enforced against the producer files themselves rather than trusted to the prose here: `TestEveryProducerStampsOrigin` owns `Origin`, and `TestOnlyTheJustfileOmitsVersion` owns `Version` — the Justfile must not stamp it, `scripts/install.sh` and goreleaser must (`internal/version/stamp_sites_test.go`). `Info` carries `Version`, `Commit`, `Date`, `IsDev`, `FromSource` and `Schema`; every field but `FromSource` is on the wire, as `version/commit/date/is_dev/schema_support` (`version.go:78-110`), where the supported schema range is derived from the embedded migration registry at call time: min = baseline, max = highest migration (`Get`, `version.go:128-141`). `BuildAge` is zero/false for an empty, unparseable, or future date (`version.go:149-162`); `StaleSourceBuild` returns that age plus the one staleness verdict every surface reads (`version.go:187-190`).

`lit version` (`internal/cli/version.go:17-66`) rejects positionals (`usage: lit version`); prints `lit <version> (commit <c>, built <d>)` with `dev`/`unknown` fallbacks; then `built <age> ago` when the age is known; then, when `Info.StaleSourceBuild` reports true — a from-source build whose age is at or past the 7-day `StaleBuildThreshold`, so a release build never warns at any age — ``WARNING: this build is at least <threshold> old — run `just build` (or `just install`) to refresh``, rendered by the pure `versionStalenessWarning` (`version.go:88-96`) through the same `stalenessThresholdClause` and `buildRefreshRemedy` its sibling surfaces use; and finally `schema versions supported: <min>–<max>`.

## Release distribution

### Manifest

Each release publishes `release-manifest.json`: the `version.Info` fields plus an `artifacts` array of `{platform: "<goos>/<goarch>", url, sha256}` and a reserved, currently-unverified `signature` (`internal/release/manifest.go:42-46`, `:54-58`, `:65-68`). `is_dev` always serializes false for a published manifest (a release is by definition not a dev build) and stays on the wire; `from_source` never appears on the wire at all, in any value — `Info.FromSource` is tagged `json:"-"` at its declaration (`internal/version/version.go:108`), because the question is meaningless for a release and a manifest carrying the field would fail to decode in every lit already installed. `TestManifestNeverSerializesFromSource` asserts the marshalled bytes, not the tag.

### Resolution

`HTTPResolver` fetches `<base>/<tag>/release-manifest.json` from `https://github.com/promptctl/links-issue-tracker/releases/download` (`internal/release/resolver.go:69`, `:164-172`). Tag validation: must start with `v`, must match `^v[A-Za-z0-9._+-]+$`, must not contain `..` (`acceptTag`, `resolver.go:100-111`). 60s default HTTP timeout; non-200 errors carry the first 256 body bytes; the body is decoded through a 1 MiB limit that **tolerates unknown fields** — `lit upgrade` is run by the installed binary to discover a newer release, so a manifest's consumer is by construction older than its producer and a field the producer added later is the ordinary case, not an attack (`TestHTTPResolverToleratesFieldsFromANewerProducer`); trailing JSON after the manifest is still rejected (`resolver.go:211-229`). No key-set check replaces it on the producer side: release-validate's "Assert manifest shape" step asserts only that the required fields are present and well-formed (`.version` a non-empty string, `.schema_support.min`/`.max` numbers, every artifact's `platform`, `url` and `sha256` matching their patterns with the tag segment present in the URL) and that the artifact platform set equals the release contract exactly. Artifact selection is an exact platform match against `runtime.GOOS + "/" + runtime.GOARCH`; a miss lists the available platforms (`internal/release/target.go:28-46`).
