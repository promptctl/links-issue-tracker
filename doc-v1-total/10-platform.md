...
Link-time variables `Version`, `Commit`, `Date`, `Origin` are stamped by three writers: goreleaser (all four, `Origin` the literal `release`), `scripts/install.sh` source mode (all four, `Version` via `git describe`), and the Justfile `build` recipe (Commit + Date + Origin only — a `just build` binary is a dev build) (`internal/version/version.go:23-45`). Two of those four fields are enforced against the producer files themselves rather than trusted to the prose here: `TestEveryProducerStampsOrigin` owns `Origin`, and `TestOnlyTheJustfileOmitsVersion` owns `Version` — the Justfile must not stamp it, `scripts/install.sh` and goreleaser must (`internal/version/stamp_sites_test.go`). `Info` carries `version/commit/date/is_dev/from_source/schema_support`, where the supported schema range is derived from the embedded migration registry: min = baseline, max = highest migration (`version.go:77-125`). `BuildAge` is zero/false for an empty, unparseable, or future date (`version.go:133-146`); `StaleSourceBuild` returns that age plus the one staleness verdict every surface reads (`version.go:171-174`).

`lit version` (`internal/cli/version.go:17-75`) rejects positionals (`usage: lit version`); prints `lit <version> (commit <c>, built <d>)` with `dev`/`unknown` fallbacks; then `built <age> ago` when the age is known; then, when `Info.StaleSourceBuild` reports true — a from-source build whose age is at or past the 7-day `StaleBuildThreshold`, so a release build never warns at any age — a warning to rebuild; and finally `schema versions supported: <min>–<max>`.
