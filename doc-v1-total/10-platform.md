...
`UpstreamRemote` (first segment of `@{upstream}`), `RemoteHasRefs` (`git ls-remote` non-empty), `RemoteHasDoltData` (`git ls-remote <r> refs/dolt/*` non-empty), `DefaultRemoteBranch` (`symbolic-ref` on the remote-HEAD ref, falling back to `ls-remote --symref`), `GitRemotes` (`git remote -v` fetch lines, deduped, sorted). A blank remote name normalizes to `origin` (`workspace.go:141-181, 401-470`).
