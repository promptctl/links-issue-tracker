...
- The status *filter* matches the **derived** state (an epic's computed state); search is case-insensitive substring across exactly four fields — title, description, prompt, topic — and multiple search terms are conjunctive (`internal/storage/selects.go:102-142`).
...
**Create** (`memory/issues.go:28-105`): parent resolved before prefix (so a missing parent reports the missing issue); title required after trim; labels canonicalized (normalize, dedupe, sort); topic normalized; type defaults to `task`; all string fields trimmed; new issues start `open`/live. Top-level ids are minted with the adaptive hash (lengths tried up to 8, 10 nonces each); children of a parent get sequential `<parent>.N` ids counting only direct children. Placement zero-value appends to the whole order (bottom); `RankTop` leads the frame the issue is filed into — the live parent it is created under, or the top level — and, when that frame holds nothing yet, lands immediately after the issue that frames it, or at slot `0` when that frame is the top level. The create event records `status "" → "open"` for a leaf and no changes for a container.
