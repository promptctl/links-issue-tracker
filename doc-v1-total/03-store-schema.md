...
Default placement is bottom (the `RankPlacement` zero value): rank = `After(max live rank)`, or the initial rank `"V"` in an empty workspace; `RankTop` takes the midpoint between the leading rank of the frame the issue is filed into (the live parent named at create, or the top level) and the nearest rank the whole workspace holds below that leader — `Before(leader)` when the workspace holds no rank below it. A frame holding nothing ranked yet offers no leader to measure against, so the key lands just past the rank of the issue that contains it: the midpoint between the container's own rank and the next rank above it, or `After(container rank)` when nothing sorts above it. At the top level, which holds nothing ranked only when the whole workspace does, both bounds are open and the key is `rank.Initial()`. Bounds leaving no room between them make the store respace the ranks around the upper bound and read both bounds again (`store.go:2088-2096`; `internal/store/ranking.go:212-221`, `:173-200`, `:920-938`). Consecutive default creates therefore keep authoring order.
