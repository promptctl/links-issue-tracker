...
- `lit workflows` is live: every dispatched event now matches against the loaded definition set (project → global → embedded defaults) and injects every match's body — with `<id>` interpolated to the acted-on ticket — into that command's output, in stable `id` order. `done`'s post-close capture reminder is now the first embedded default (`events: [work_finished]`), with the same observable output as before.
