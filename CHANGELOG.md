...
- CI license-policy gate: every linked module's license is checked against a committed allowlist (`tools/licenses/policy.json`) of permissive licenses plus documented per-module exceptions, and the build fails if a dependency bump pulls in a non-allowlisted license (GPL/AGPL/SSPL/BUSL, etc.). Runs on every PR via `go test` and on the release pipeline via `go run ./tools/licenses -check`. Shares the classifier with the license report/SBOM, so the gate checks the exact licenses those artifacts document.
