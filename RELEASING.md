...
1. **In the `chore(release)` PR:** rename `## [Unreleased]` in
   ...
   line under [`design-docs/`](design-docs/) to `built (v<version>)` — the version
   becomes a fact at this commit and nowhere earlier, so this is the one place the
   two can be written down together, and it is why a design doc never names the
   release it *expects* to ship in.
